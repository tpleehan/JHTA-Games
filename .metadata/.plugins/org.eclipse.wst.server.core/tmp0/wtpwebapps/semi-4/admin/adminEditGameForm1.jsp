...
%>
					<h3 class="text-center" style="font-weight: bold;">게임 정보 수정/삭제</h3><br><br>
					<div class="row">
						<div class="col-sm-offset-1 col-sm-10">
						<h4 class="text text-primary"><strong>수정할 게임을 선택하세요</strong></h4>
							<form class="well" method="post" id="edit-form" action="">
							<div class="form-group">
								<label>게임 선택</label>
								<select class="form-control inputbox" name="gameno" id="game-no">
	<%
	...
	%>
								...
								<option value="<%=game.getNo() %>"><%=game.getTitle() %></option>
	<%
		}
	%>
								</select>
							...
							<div class="text-right">
								<button onclick="editGame(event)" class="btn btn-primary" id="edit-button">게임 수정</button>
								<button onclick="deleteGame(event)" class="btn btn-danger" id="del-button">게임 삭제</button>
							</div>
						</form>
						</div>
					</div>
...
<script type="text/javascript">
	function editGame(e) {
		document.querySelector("#edit-form").action = "adminEditGameForm2.jsp";
		document.querySelector("#edit-form").submit;
	}
	
	function deleteGame(e) {
		document.querySelector("#edit-form").action = "adminDeleteGameForm.jsp";
		document.querySelector("#edit-form").submit;
	}
