	...
	<!-- 구분선 -->
			...
			<div class="row">
				<div class="col-sm-offset-1 col-sm-10">
					<form class="well" method="post" id="edit-form" enctype="multipart/form-data" action="adminEditGame.jsp">
						<div class="form-group">
							<input type="hidden" class="form-control" name="gameno" value="<%=game.getNo() %>" />
						</div>
						<div class="form-group">
							<label>게임 제목</label>
							<input type="text" class="form-control" name="title" id="game-title" value="<%=game.getTitle() %>" onkeypress="checkTitle()" />
						</div>
						<div class="form-group">
							<label>가격</label>
							<input type="text" class="form-control" name="price" id="game-price" value="<%=game.getPrice() %>" onkeypress="numberOnly()"/>
						</div>
						<div class="form-group">
							<label>할인가격</label>
							<input type="text" class="form-control" name="discountPrice" id="game-discountPrice" value="<%=game.getDiscountPrice() %>" onkeypress="numberOnly()"/>
						</div>
						<div class="form-group">
							<label>출시일</label>
							<input type="date" class="form-control" name="createDate" id="game-createDate" value="<%=date %>" />
						</div>
						<div class="form-group">
							<label>장르 선택</label>
...
%>
								...
								<option value="<%=genre.getNo() %>" <%=genre.getNo() == genreNo ? "selected" : "" %>><%=genre.getName() %></option>
<%
	}
%>
							</select>
						...
						<div class="form-group">
							<label>퍼블리셔 선택</label>                                        
							<select class="form-control inputbox" name="publisherno" onchange="changePublisher()" id="pub-select">
							<%
							...
							%>
								<option value="<%=publisher.getNo() %>" <%=publisher.getNo() == publisherNo ? "selected" : "" %> ><%=publisher.getName() %></option>
							<%
								}
							%>
								<option value="0">직접입력</option>
							</select>
							<input type="text" name="publishername" class="form-control" id="publisher-input-form" readonly/>
						</div>
						<div class="form-group">
							<label>썸네일 이미지</label>
							<p style="padding:10px;"><img style=" width:100%; height:50%;" alt="" 
							src="../resources/images/gameImages/<%=game.getImageFileName() %>"></p>
...
<!-- 구분선 ------------------------------------------------------------------------------------------ -->						
					...
					%>
						<div id="image-item-<%=imageNumber %>">
							<p style="padding:10px;">
								<input class="form-control hidden" name="imageno<%=imageNumber %>" value="<%=image.getNo() %>">
								<img style=" width:100%; height:50%;" alt="" src="../resources/images/gameImages/<%=image.getImage() %>">
							</p>
							<input type="file" class="form-control" name="detailImage<%=imageNumber %>" style="margin-bottom: 15px; margin-top: -15px;" />
						</div>
					<% 
						}
					%>
					...
					<input class="hidden" type="number" id="image-count" name="imagecount" value="<%=imageNumber %>">
							
<!-- 구분선 ------------------------------------------------------------------------------------------ -->							
						</div>
						<div class="form-group">
							<label>게임 소개</label>
							<textarea class="form-control" style="width: 100%; min-height:300px; resize: none; border: 1;" name="intro" id="game-intro" ><%=game.getIntro() %></textarea>
						</div>
						<div class="form-group">
							<label>시스템 요구 사항</label>
							<textarea class="form-control" style="width: 100%; min-height:300px; resize: none; border: 1;" name="requirement" id="game-requirement"><%=game.getRequire() %></textarea>
						</div>
