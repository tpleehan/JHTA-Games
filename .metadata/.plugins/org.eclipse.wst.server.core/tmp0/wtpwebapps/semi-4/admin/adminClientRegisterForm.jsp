...
%>
	<div class="row">
		<div class="col-xs-12 text-center" style="height:500px;" >
			<div class="alert alert-danger" style="height:200px;display:table-cell;vertical-align:middle; width:800px;">
				<p><strong>관리자 권한 제한</strong></p>
				<p>일반등급 관리자는 관리자 계정 생성을 할 수 없습니다.</p>
			</div>
		</div>
	</div>
<%		
	...
	if("samename".equals(result)) {
%>
	<div class="row">
		<div class="col-xs-12 text-center">
			<div class="alert alert-danger">
				<p><strong>동일 아이디 존재</strong></p>
				<p>이미 존재하는 아이디입니다.</p>
			</div>
		</div>
	</div>
	...
	<div class="row">
		<div class="col-xs-12 text-center">
			<div class="alert alert-success">
				<p><strong>관리자 아이디 생성 완료</strong></p>
				<p>관리자 아이디 생성이 완료되었습니다.</p>
			</div>
		</div>
	</div>
<%
...
%>
				<div class="row">
					<div class="col-sm-12">
					<h3 class="text-center" style="font-weight: bold;">관리자 새로 등록하기</h3><br><br>
				</div>
				<div class="row">
					<div class="col-sm-offset-1 col-sm-10">
						<form class="well" method="post" id="register-form" action="adminClientRegister.jsp" >
						<div class="form-group">
							<label>아이디를 입력하세요</label>
							<input type="text" class="form-control" name="adminid" id="admin-id" />
						</div>
						<div class="form-group">
							<label>비밀번호를 입력하세요</label>
							<input type="password" class="form-control" name="adminpwd" id="admin-password" />
						</div>
						<div class="form-group">
							<label>이름을 입력하세요</label>
							<input type="text" class="form-control" name="adminname" id="admin-name" />
						</div>
						<div class="form-group">
							<label>관리자 등급을 선택하세요</label>
							<select class="form-control inputbox" name="admingrade" id="admin-grade">
								<option value="S">상위등급 관리자 계정 </option>
								<option value="N">일반등급 관리자 계정 </option>								
							</select>
						</div>
						<div class="text-right">
							<button onclick="checkFormField()" type="button" class="btn btn-primary" >관리자 등록</button>
						</div>
						</form>
					</div>
				</div>
...
<script type="text/javascript">
	function checkFormField() {
		//관리자 아이디 공란 확인
		var adminid = document.querySelector("#admin-id").value;
		if(adminid == "") {
			alert("관리자 아이디를 입력하세요");
			return;
		}
		//관리자 비밀번호 공란 확인
		var password = document.querySelector("#admin-password").value;
		if(password == "") {
			alert("비밀번호를 입력하세요");
			return;
		}
		//관리자 이름 공란 확인
		var adminName = document.querySelector("#admin-name").value;
		if(adminName == "") {
			alert("이름을 입력하세요");
			return;
		}
