				...
				<div class="col-sm-12" style="border-bottom: 1px solid grey;">
					...
					%>
								...
								<tr>
									...
									<th style="padding-bottom: 50px;"><%=board.getCount() %></th>
								</tr>
						...
						<form id="qnaAnswer-form" action="adminQnaEdit.jsp" method="post">
							<button class="btn btn-primary btn-xs" style="padding: 7px; margin-left: 20px; margin-bottom: 10px;">답변내용</button>
							<label style="margin-left: 50px;">답변자 : <%=loginedAdmin.getName() %></label>
								<div class="col-sm-12">
									...
									<textarea class="form-control" id="answer" rows="8" style="width: 100%; resize: none; border: 1px solid grey; font-weight: bold; color: black;" 
									name="answer" onclick="answercheck()"><%=answer == null ? "답변이 없습니다." : answer.getContent() %></textarea>
						...
						<% 
							if("all".equals(where)) {
						%>
							<a href="adminQna.jsp"><button class="btn btn-success pull-right" type="button" style="margin: 20px 10px;">목록으로</button></a>
						<%		
							}else if("completed".equals(where)) {
						%>
							<a href="adminQnaAnswerCompleted.jsp"><button class="btn btn-success pull-right" type="button" style="margin: 20px 10px;">목록으로</button></a>
						<% 		
							}else if("ready".equals(where)) {
						%>
							<a href="adminQnaNoAnswer.jsp"><button class="btn btn-success pull-right" type="button" style="margin: 20px 10px;">목록으로</button></a>
						<% 		
							}
						%>	
								<button class="btn btn-primary pull-right" type="button" onclick="doAnswer()" style="margin: 20px 10px;">답변/수정</button>
								<button class="btn btn-danger pull-right" type="button" onclick="deleteqna(<%=qnaNo %>)" style="margin: 20px 10px;">삭제</button>
							</div>
						</form>
					</div>
				</div>
